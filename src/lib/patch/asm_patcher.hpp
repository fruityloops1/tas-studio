...
#define PATCH_DEFINE_ASM(name, assembly)                                                                                \
    ...
    {                                                                                                                   \
        ...
        exl::replace::ReplaceData((uintptr_t)AddrRef(), (uintptr_t)OrigData, GetAsmSize());                             \
    }                                                                                                                   \
                                                                                                                        \
...
#define PATCH_DEFINE_ASM_SIMPLE(name, assembly)                                                                         \
    asm(".global __asm" #name "\n"                                                                                      \
        "__asm" #name ":\n" assembly);                                                                                  \
    asm(".global __asm" #name "end\n"                                                                                   \
        "__asm" #name "end:\nnop\n");                                                                                   \
    ...
    static ALWAYS_INLINE void InstallAtOffset(ptrdiff_t address)                                                        \
    {                                                                                                                   \
        exl::replace::ReplaceData(exl::util::modules::GetTargetOffset(address), (uintptr_t) __asm##name, GetAsmSize()); \
    }                                                                                                                   \
                                                                                                                        \
    template <typename R, typename... A>                                                                                \
    static ALWAYS_INLINE void InstallAtFuncPtr(exl::util::GenericFuncPtr<R, A...> ptr)                                  \
    {                                                                                                                   \
        exl::replace::ReplaceData(*reinterpret_cast<uintptr_t*>(&ptr), (uintptr_t) __asm##name, GetAsmSize());          \
    }                                                                                                                   \
    }
